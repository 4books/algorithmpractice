...
public class Main {
    ...
    static class Position {
        ...
        public int getDis(Position target) { //거리 구하기
    ...
    static String solution(int[] numbers, String hand) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            int next = numbers[i];
            Position curr = keypad.get(numbers[i]);
            if (next == 1 || next == 4 || next == 7) {
                answer.append("L");
                left.changePosition(curr);
            } else if (next == 3 || next == 6 || next == 9) {
                answer.append("R");
                right.changePosition(curr);
            } else { //가운데 버튼들을 누를 경우
                int leftDis = left.getDis(curr);
                int rightDis = right.getDis(curr);

                if (leftDis > rightDis) { //오른손이 더 가까운 경우
                    answer.append("R");
                    right.changePosition(curr);
                } else if (leftDis < rightDis) { //왼손이 더 가까운 경우
                    answer.append("L");
                    left.changePosition(curr);
                } else { //둘다 거리가 같은 경우
