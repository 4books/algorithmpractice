...
public class Main {
    ...
    public static String solution(int n, int[][] grid, int moveCount, int[] moves) {
        int answer = 0;
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = Integer.parseInt(br.readLine()); //grid 가로 세로 수
        int[][] grid = new int[n][n];

        for (int i = 0; i < n; i++) {
            String[] s = br.readLine().split(" ");
            for (int j = 0; j < s.length; j++) {
                int doll = Integer.parseInt(s[j]);
                grid[i][j] = doll;
            }
        }
        ...
        bw.write(solution(n, grid, moveCount, moves));
        bw.flush();
        bw.close();
    }
}
