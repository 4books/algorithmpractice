package programers.level2.p42888;
/*
    ...
    static public String[] solution(String[] record) {
        ...
        for (String r : record) {
            ...
            if (tmp[0].equals("Enter")) {
                ...
                idNameMap.put(tmp[1], tmp[2]); //아이디 / 닉네임 추가
        ...
        int resultSize = chatLog.size();
        String[] answer = new String[resultSize];

        //아이디를 마지막 닉네임으로 변경
        for (int i = 0; i < resultSize; i++) {
            String chat = chatLog.get(i);
            int endIdIndex = chat.indexOf("님");
            String id = chat.substring(0, endIdIndex);

            answer[i] = chat.replace(id, idNameMap.get(id));
        }
