...
public class Main {
    ...
    static class Player implements Comparable<Player>{
        ...
        @Override //양수가 나오면 바꾼다. 키 순으로 내림차순
    ...
    public static String solution() {
        ...
        for (Player player : playerList) {
            if(player.weight > weightMax) {
                weightMax = player.weight;
                answer++;
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            playerList.add(new Player(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1])));
        }
